...
| Problème | Solution |
|----------|----------|
| `sh: next: not found` / `standalone/server.js` introuvable | Vérifier **Root Directory** = `frontend` et que le build tourne (`buildCommand` dans `railway.toml`). Le démarrage utilise `next start`. |
